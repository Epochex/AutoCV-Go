...
一个面向中文网申场景的本地优先 Chrome / Edge 扩展。保存一次简历资料后，扩展会识别招聘页面中的空字段，按规则匹配并在人工可控的前提下填写；规则无法确定时，可以选择使用自己的 OpenAI 兼容 API 完成字段映射或简历解析。
...
这种方式不会为了姓名、邮箱等明确字段反复消耗 Token。表单字段映射不会发送简历具体值；只有用户在文件导入区主动勾选 AI 解析时，才会发送提取出的简历全文。

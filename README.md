...
- 扫描会列出全部待填写字段；未匹配项可从本地资料中选择、编辑、复制或应用，复杂控件手动完成后也可检查并记住映射；
- 用户确认且成功填写的字段映射会形成可撤销、可清空的本地记忆，后续同站点优先复用；
...
- 部分自绘下拉框、跨域 iframe 和 closed Shadow DOM 需要站点适配器；开放 Shadow DOM 已纳入扫描；
- 页面动态添加经历后，建议重新扫描；
